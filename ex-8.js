...
let totalBill = bills
  ...
  .map((locationName) => {
    return `${locationName.location} : ${locationName.total}`;
  });
console.log(totalBill);
// ถ้ามันอยู่ในรูปสตริง {แมว : เมี้ยว} ก็ split ':' ?

const uniqueTotalBill = totalBill.reduce((acc, curr) => {
  const [location, total] = curr.split(" : ");
  console.log(location);
  console.log(typeof location); // all string
  console.log(total);
  console.log(typeof total); // also all string
  //   acc[location] = Number(total);
  if (acc[location] == undefined) {
    acc[location] = Number(total);
  } else {
    acc[location] = acc[location] + Number(total);
  }
  return acc;
}, {});

console.log(uniqueTotalBill);
